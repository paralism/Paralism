...
struct property_proxy
{
    ...
    property_proxy& operator =(T value)
    {
        js_property_traits<Key>::set_property(ctx, this_obj, key,
                                              js_traits<T>::wrap(ctx, std::move(value)));
        return *this;
    }
...
public:
    ...
    JSValue release() // don't call JS_FreeValue
    {
        ctx = nullptr;
        return v;
    }
    ...
    add(const char * name)
    {
        auto fn =  fwrapper<F>{name};
        (*this)[name] = js_traits<decltype(fn)>::wrap(ctx, fn); //fwrapper<F>{name};
        return *this;
    }
...
public:
    ...
    {
        std::stringstream ss;
        auto excp = getException();
        ss << (std::string) excp << std::endl;
        if ((bool)excp["stack"])
            ss << (std::string) excp["stack"] << std::endl;
        return ss.str();
    }
    ...
    Value evalBinary(const char* buff, size_t bufflen)
    {
        std::string_view buf_view = { reinterpret_cast<const char*>(buff), bufflen };
        return evalBinary(buf_view);
    }

    Value evalBinary(std::string_view buf)
    {
        JSValue obj, val;

        obj = JS_ReadObject(ctx, reinterpret_cast<const uint8_t*>(buf.data()), buf.size(),
    ...
    Value evalFile(const char * filename, bool isbinaryfile, unsigned eval_flags = 0)
    {
        size_t buf_len;
        ...
        if(isbinaryfile)
            return evalBinary({ reinterpret_cast<char*>(buf.get()), buf_len });
        return eval({reinterpret_cast<char *>(buf.get()), buf_len}, filename, eval_flags);
    }

    std::string dumpObject(Value &obj)
    {
        uint8_t* out_buf;
        size_t out_buf_len;
        int flags = JS_WRITE_OBJ_BYTECODE;

        JSValue val = obj.as<JSValue>();
        ...
        JS_FreeValue(ctx, val);
        std::string ret((char*)out_buf, out_buf_len);
        js_free(ctx, out_buf);
        return ret;
    }

    std::string dumpString(const char* str, size_t len)
    {
        uint8_t* out_buf;
        size_t out_buf_len;

        out_buf = JS_StrFormatWriteStr(ctx, &out_buf_len, str, len);
        if (!out_buf) {
            throw qjs::exception{};
        }
...
struct js_traits<std::function<R(Args...)>>
{
    static auto unwrap(JSContext * ctx, JSValueConst fun_obj)
    {
        return [jsfun_obj = Value{ctx, JS_DupValue(ctx, fun_obj)}](Args&& ... args) -> R {
            ...
            for(int i = 0; i < argc + 1; i++) JS_FreeValue(jsfun_obj.ctx, argv[i]);
