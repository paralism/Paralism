...
int ec_seckey_export_der(const secp256k1_context *ctx, unsigned char *seckey, size_t *seckeylen, const unsigned char *key32, bool compressed) {
    assert(*seckeylen >= CKey_Secp256k1::SIZE);
    ...
    if (compressed) {
        ...
        *seckeylen = ptr - seckey;
        assert(*seckeylen == CKey_Secp256k1::COMPRESSED_SIZE);
    } else {
        static const unsigned char begin[] = {
...
bool CKey_Secp256k1::Check(const unsigned char *vch) {
    return secp256k1_ec_seckey_verify(secp256k1_context_sign, vch);
}

void CKey_Secp256k1::MakeNewKey(bool fCompressedIn) {
    do {
...
CPubKey CKey_Secp256k1::GetPubKey() const {
    assert(fValid);
    secp256k1_pubkey pubkey;
...
bool CKey_Secp256k1::Sign(const uint256 &hash, std::vector<unsigned char>& vchSig, bool grind, uint32_t test_case) const {
    if (!fValid)
        return false;
...
bool CKey_Secp256k1::VerifyPubKey(const CPubKey& pubkey) const {
...
bool CKey_Secp256k1::SignCompact(const uint256 &hash, std::vector<unsigned char>& vchSig) const {
    if (!fValid)
        return false;
...
bool CKey_Secp256k1::Load(const CPrivKey &seckey, const CPubKey &vchPubKey, bool fSkipCheck=false) {
...
bool CKey_Secp256k1::Derive(CKey_Secp256k1& keyChild, ChainCode &ccChild, unsigned int nChild, const ChainCode& cc) const {
    assert(IsValid());
    assert(IsCompressed());
