...
bool HidingSigningProvider::GetKey(const CKeyID& keyid, CKey_Secp256k1& key) const
{
...
bool FlatSigningProvider::GetKey(const CKeyID& keyid, CKey_Secp256k1& key) const { return LookupHelper(keys, keyid, key); }
...
bool FillableSigningProvider::GetPubKey(const CKeyID &address, CPubKey &vchPubKeyOut) const
{
    ...
    vchPubKeyOut = CPubKey(key.GetPubKey());
    return true;
}

bool FillableSigningProvider::AddKeyPubKey(const CKey_Secp256k1& key, const CPubKey &pubkey)
{
    LOCK(cs_KeyStore);
...
bool FillableSigningProvider::GetKey(const CKeyID &address, CKey_Secp256k1 &keyOut) const
{
    LOCK(cs_KeyStore);
...
bool FillableSigningProvider::AddCScript(const CScript& redeemScript)
{
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE)
        return false; //HCE: error("FillableSigningProvider::AddCScript(): redeemScripts > %i bytes are invalid", MAX_SCRIPT_ELEMENT_SIZE);
