...
public:
    ...
    bool operator==(BlockHeader const& _cmp) const
    {
        return m_parentHash == _cmp.parentHash() &&
            ...
            m_prevHID == _cmp.prevHID() &&
            m_prevHyperBlockHash == _cmp.prevHyperBlkHash()
            ;
    }
    ...
    void setPrevHID(int64_t _v) { m_prevHID = _v; noteDirty(); }
    void setHashPrevHyperBlock(u256 const& _v) { m_prevHyperBlockHash= _v; noteDirty(); }
    ...
    int64_t prevHID() const { return m_prevHID; }
    h256 const& prevHyperBlkHash() const { return m_prevHyperBlockHash; }
...
inline std::ostream& operator<<(std::ostream& _out, BlockHeader const& _bi)
{
    _out << _bi.hash(WithoutSeal) << " " << _bi.parentHash() << " " << _bi.sha3Uncles() << " " << _bi.author() << " " << _bi.stateRoot() << " " << _bi.transactionsRoot() << " " <<
        ...
        _bi.prevHID() << " " << _bi.prevHyperBlkHash();
    return _out;
}
