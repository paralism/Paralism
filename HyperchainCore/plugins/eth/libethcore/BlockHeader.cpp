...
BlockHeader::BlockHeader(BlockHeader const& _other) :
    ...
    m_seal(_other.seal()),
    m_prevHID(_other.prevHID()),
    m_prevHyperBlockHash(_other.prevHyperBlkHash()),
...
void BlockHeader::streamRLPFields(RLPStream& _s) const
{
    _s	<< m_parentHash << m_sha3Uncles << m_author << m_stateRoot << m_transactionsRoot << m_receiptsRoot << m_logBloom
        ...
        << m_prevHID << m_prevHyperBlockHash;
}
...
void BlockHeader::populate(RLP const& _header)
{
    ...
    try
    {
        ...
        m_prevHID = _header[field = 13].toInt<uint32_t>();
        m_prevHyperBlockHash = _header[field = 14].toHash<h256>();
        m_seal.clear();
...
void BlockHeader::populateFromParent(BlockHeader const& _parent)
{
    ...
    setHyperBlockInfo();
    LOG(m_logger) << "populateFromParent: Previous HyperBLock : " << m_prevHID
        << " New Block height: " << m_number;
}

void BlockHeader::setHyperBlockInfo()
{
    m_prevHID = LatestHyperBlock::GetHID(&m_prevHyperBlockHash);
}
