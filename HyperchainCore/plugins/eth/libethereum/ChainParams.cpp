...
void ChainParams::loadGenesis(string const& _json, h256 const& _stateRoot)
{
    ...
    previousHHash = genesis.count(c_previousHHash) ?
                     h256(genesis[c_previousHHash].get_str()) :
                     h256();
...
void ChainParams::populateFromGenesisWithoutCalc(bytes const& _genesisRLP)
{
    ...
    stateRoot = bi.stateRoot();  //HC：直接赋值
    previousHID = bi.prevHID();
    previousHHash = bi.prevHyperBlkHash();
...
{
    RLPStream block(3); //HC：准备压入3个List
