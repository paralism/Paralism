...
std::string currentTimeStr()
{
    ...
    return StringFormat("%d-%02d-%02d %02d:%02d:%02d.%03d", tmNow.tm_year + 1900,
    ...
    if (badBlockHashes.size()) {
        for (h256 const& b : badBlockHashes) {
            if (b == m_working.info().hash()) {
                resetWorking();
                LOG(m_logger) << b << " Working is a bad block, resetWorking() *******************\n";
                break;
            }
        }
    }
...
void Client::resetWorking() {
    ...
    bool expected = false;
    m_syncBlockQueue.compare_exchange_strong(expected, true);

    DEV_READ_GUARDED(x_working)
        LOG(m_logger) << "new working block:" << m_working.info().hash()
            << " number:" << m_working.info().number()
            << " prevHID:" << m_working.info().prevHID()
            << " prevHyperBlkHash:" << m_working.info().prevHyperBlkHash()
            << endl;
};
...
void Client::restartMining(bool _isForce)
{
    ...
    DEV_READ_GUARDED(x_preSeal) DEV_READ_GUARDED(x_postSeal)
            if (!_isForce && !preChanged && m_preSeal.author() == m_postSeal.author())
    {
