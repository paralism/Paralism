...
inline uint8_t nibble(bytesConstRef _data, unsigned _i)
{
...
struct NibbleSlice
{
	...
	uint8_t operator[](unsigned _index) const { return nibble(data, offset + _index); }
...
uint8_t uniqueInUse(RLP const& _orig, uint8_t except);
