...
public:
    ...
    private:
        ...
        struct Node
        {
            ...
            uint8_t child;				// 255 -> entering, 16 -> actually at the node, 17 -> exiting, 0-15 -> actual children.
...
private:
    ...
    // out2: [k{}, S]      where i == 16
    bytes merge(RLP const& _orig, uint8_t _i);
...
public:
    ...
    bool contains(KeyType _k) const { return Generic::contains(bytesConstRef((uint8_t const*)&_k, sizeof(KeyType))); }
    std::string at(KeyType _k) const { return Generic::at(bytesConstRef((uint8_t const*)&_k, sizeof(KeyType))); }
    void insert(KeyType _k, bytesConstRef _value) { Generic::insert(bytesConstRef((uint8_t const*)&_k, sizeof(KeyType)), _value); }
    void insert(KeyType _k, bytes const& _value) { insert(_k, bytesConstRef(&_value)); }
    void remove(KeyType _k) { Generic::remove(bytesConstRef((uint8_t const*)&_k, sizeof(KeyType))); }
    ...
    iterator end() const { return iterator(); }
    iterator lower_bound(KeyType _k) const { return iterator(this, bytesConstRef((uint8_t const*)&_k, sizeof(KeyType))); }
};
...
template <class DB> bytes GenericTrieDB<DB>::deleteAt(RLP const& _orig, NibbleSlice _k)
{
    ...
    else
    {
        ...
        if (_k.size() == 0 && !_orig[16].isEmpty())
        {
            ...
            uint8_t used = uniqueInUse(_orig, 16);
            if (used != 255)
        ...
        else
        {
            ...
            RLP rlp(r.out());
            uint8_t used = uniqueInUse(rlp, 255);
...
template <class DB> bytes GenericTrieDB<DB>::merge(RLP const& _orig, uint8_t _i)
{
