...
Json::Value Eth::eth_recvCrossChainTransaction(Json::Value const& _json)
{
    Json::Value res;
    h256 tx_hash;
    try {
        TransactionSkeleton t = toTransactionSkeleton(_json);
        t.crosschain_recv = true;

        bytes keytmp = fromHex(_json["senderprikey"].asString());
        bytesSec key = bytesSec(keytmp);
        
        tx_hash = client()->submitTransaction(t, Secret(key));
    }
    catch (TransactionAlreadyInChain const& e) {
        tx_hash = *boost::get_error_info<errinfo_hash256>(e);
    }
    catch (PendingTransactionAlreadyExists const& e) {
        tx_hash = *boost::get_error_info<errinfo_hash256>(e);
    }
    catch (Exception const& e) {
        auto ptrtxhash = boost::get_error_info<errinfo_hash256>(e);
        auto comm = boost::get_error_info<errinfo_comment>(e);
        string err = exceptionToErrorMessage();

        if (ptrtxhash) {
            err += string("Tx:") + (*ptrtxhash).hex();
        }
        if (comm) {
            err += string(" ") + comm->c_str();
        }
        throw JsonRpcException(err);
        //throw JsonRpcException(e.what());
    }

    res["hash"] = tx_hash.hex();
    return res;
}

Json::Value Eth::eth_sendCrossChainTransaction(Json::Value const& _json)
{
    Json::Value res;
    try {
        TransactionSkeleton t = toTransactionSkeleton(_json);
        t.crosschain_send = true;

        setTransactionDefaults(t);
        pair<bool, Secret> ar = m_ethAccounts.authenticate(t);
        if (!ar.first) {
            Secret s = ar.second;
            h256 tx_hash = client()->submitTransaction(t, s);

            res["hash"] = tx_hash.hex();  //toJS(tx_hash);

            //HC: 
            Signature signature = dev::sign(s, tx_hash);
            //res["signature"] = toJS(signature);

            Public pubkey = dev::recover(signature, tx_hash);
            res["publickey"] = toJS(pubkey);
            return res;
        }
        throw JsonRpcException(string("not find secret of the sender"));
    }
    catch (Exception const&) {
        throw JsonRpcException(exceptionToErrorMessage());
    }
...
string dev::rpc::exceptionToErrorMessage()
{
    ...
    catch (IllegalCrossChainTransaction const&)
    {
        ret = "Illegal Cross-Chain Transaction.";
    }
    catch (...)
    {
