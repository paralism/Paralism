...
//  double blockFees = 15.0;
    ...
    bool isBuiltIn = g_cryptoEthCurrency.isNullCurrency();

    //fs::path configFile = getConfigFile(); //getDataDir() / fs::path("config.rlp");
    ...
    if (!configJSON.empty())
    {
        try
        {
            chainParams = std::make_shared<ChainParams>(ChainParams{configJSON, {}, configPath});
            chainConfigIsSet = true;
        }
        catch (...)
        {
    ...
    if (!chainConfigIsSet) {
        chainParams = g_cryptoEthCurrency.GetChainParams();
    } else {
        g_cryptoEthCurrency.SetChainParams(chainParams);
    }

    CallbackRegister cbReg;
    ...
    dev::WebThreeDirect web3(WebThreeDirect::composeClientVersion("aleth"), db::databasePath(),
        snapshotPath, *chainParams.get(), withExisting, netPrefs, &nodesState, testingMode);
    g_ptrWeb3 = &web3;
