...
size_t EvmCHost::copy_code(evmc::address const& _addr, size_t _codeOffset, uint8_t* _bufferData,
...
evmc::result EvmCHost::create(evmc_message const& _msg) noexcept
{
    ...
    else
    {
		...
		//constexpr int sss = sizeof(std::vector<uint8_t>);
		//constexpr int sss1 = sizeof(*data);
		//constexpr_debugger<sss>();
		//constexpr_debugger<sss1>();
